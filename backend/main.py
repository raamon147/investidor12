...
try:
    ...
    safe_ca_path = os.path.join(system_temp_dir, "cacert_investidor12.pem")
...
def get_dashboard(wallet_id: int, db: Session = Depends(get_db)):
    ...
    for tick, d in trans_map.items():
        if d["qtd"] > 0:
            ...
            ativos_finais.append({
                ...
                "rentabilidade": safe_float(rent), 
                "lucro_valor": safe_float(lucro_ativo),
                "variacao_diaria": safe_float(var_pct),
                "variacao_diaria_valor": safe_float(money_var)
            })
    ...
    if not dados_historicos.empty:
        ...
        start_ts = pd.Timestamp(data_inicio_real)
        hist_slice = dados_historicos[dados_historicos.index >= start_ts]
        ...
        ibov_start = 0
        try:
            if '^BVSP' in hist_slice.columns:
                valid = hist_slice['^BVSP'].dropna()
                if not valid.empty: ibov_start = safe_float(valid.iloc[0])
            elif isinstance(hist_slice, pd.Series) and hist_slice.name == '^BVSP':
                 ibov_start = safe_float(hist_slice.iloc[0])
        except: pass

        posicao = {t:0 for t in tickers}
        custo = 0
        trans_date = {}
        for tr in trans:
        ...
        for ts in hist_slice.index:
            ...
            if custo > 0:
                ...
                try:
                    if ibov_start > 0:
                        curr = 0
                        if isinstance(hist_slice, pd.DataFrame) and '^BVSP' in hist_slice.columns:
                            curr = safe_float(hist_slice.loc[ts]['^BVSP'])
                        elif isinstance(hist_slice, pd.Series) and hist_slice.name == '^BVSP':
                            curr = safe_float(hist_slice.loc[ts])
                        if curr > 0: rent_ibov = ((curr - ibov_start) / ibov_start) * 100
                except: pass
                
                chart_data.append({
                    "name": ts.strftime("%d/%m/%y"),
                    "carteira": safe_float(rent_cart),
                    "ibov": safe_float(rent_ibov),
                    "cdi": safe_float((cdi_acc - 1) * 100),
                    "ipca": safe_float((ipca_acc - 1) * 100)
                })
